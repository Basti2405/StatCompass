...
-- Zahl mit Tausendertrennung, z. B. 12345 -> "12.345" (deDE) / "12,345" (enUS)
local function zahl(n)
    local text = tostring(math.floor(n + 0.5))
    local davor
    repeat
        text, davor = text:gsub("^(-?%d+)(%d%d%d)", "%1" .. L.THOUSAND_SEP .. "%2")
    until davor == 0
    return text
end
...
local function ZeileBauen(eltern, index)
    ...
    z:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddDoubleLine(L.TT_RATING_GEAR, zahl(rating), 0.8, 0.8, 0.8, 1, 1, 1)
        GameTooltip:AddDoubleLine(L.TT_EFFECTIVE,
            ("%.2f %%"):format(SK.Rechner.RatingZuProzent(self.statKey, rating)), 0.8, 0.8, 0.8, 1, 1, 1)
        GameTooltip:AddDoubleLine(L.TT_CHARSHEET,
            ("%.2f %%"):format(SK.Spieler.AngezeigterWert(self.statKey)), 0.8, 0.8, 0.8, 0.7, 0.7, 0.7)

        GameTooltip:AddLine(" ")
        GameTooltip:AddLine(L.TT_ALL_LIMITS, 1, 0.82, 0)
        for _, bp in ipairs(SK.Rechner.SystemBreakpoints(self.statKey)) do
            ...
            GameTooltip:AddDoubleLine(
                (L.TT_LIMIT_LEFT):format(zahl(bp.rating), bp.rohProzent),
                (L.TT_LIMIT_RIGHT):format(bp.faktorDanach * 100),
                r, g, b, r, g, b)
        end
        GameTooltip:Show()
    end)
...
function F:Build()
    ...
    local f = CreateFrame("Frame", "StatCompassFrame", UIParent, "BackdropTemplate")
    ...
    f:SetScript("OnDragStop", function(self)
        ...
        StatCompassDB.fenster.point = point
        StatCompassDB.fenster.x = x
        StatCompassDB.fenster.y = y
    end)

    local titel = f:CreateFontString(nil, "OVERLAY", "GameFontNormalLarge")
    titel:SetPoint("TOP", 0, -12)
    titel:SetText(L.ADDON_NAME)
    ...
    f.listeTitel:SetText(L.LIST_TITLE)
    ...
    btnUpdate:SetText(L.BTN_UPDATE)
    ...
    btnTest:SetText(L.BTN_SELFTEST)
    btnTest:SetScript("OnClick", function() SK.Selbsttest() end)
    btnTest:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
        GameTooltip:AddLine(L.BTN_SELFTEST, 1, 1, 1)
        GameTooltip:AddLine(L.TT_SELFTEST_BODY, 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end)
...
-- ===========================================================================
-- Eine Zeile der Breakpoint-Liste holen (aus dem Pool)
-- ---------------------------------------------------------------------------
-- Es ist bewusst ein Frame mit einem FontString darin und nicht nur ein
-- FontString: Nur ein Frame kann Mouseover-Ereignisse empfangen, und ohne die
-- waeren die "info"-Texte aus Daten\Breakpoints.lua nirgends zu sehen -
-- gepflegt, exportiert, aber unsichtbar.
-- ===========================================================================
local function ListenZeile(f, index)
    if not f.listenZeilen[index] then
        local z = CreateFrame("Frame", nil, f)
        z:SetSize(BREITE - 2 * RAND, 14)

        z.text = z:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
        z.text:SetAllPoints()
        z.text:SetJustifyH("LEFT")

        z:EnableMouse(true)
        z:SetScript("OnEnter", function(self)
            local e = self.eintrag
            if not e then return end

            local ausPaket = (e.herkunft == "Paket")

            GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
            GameTooltip:AddLine(SK.BPText(e, "titel", ausPaket), 1, 1, 1)

            local info = SK.BPText(e, "info", ausPaket)
            if info and info ~= "" then
                GameTooltip:AddLine(" ")
                GameTooltip:AddLine(info, 0.85, 0.85, 0.85, true)
            end

            GameTooltip:AddLine(" ")
            local ziel = SK.Rechner.ZielRating(e)
            if ziel then
                GameTooltip:AddDoubleLine(L.TT_BP_TARGET,
                    ("%s  (%s)"):format(zahl(ziel), SK.STAT_NAMEN[e.stat]),
                    0.8, 0.8, 0.8, 1, 1, 1)
                GameTooltip:AddDoubleLine(L.TT_BP_CURRENT, zahl(SK.Spieler.Rating(e.stat)),
                    0.8, 0.8, 0.8, 1, 1, 1)
            end

            if e.quelle and e.quelle ~= "" then
                GameTooltip:AddLine((L.TT_BP_SOURCE):format(e.quelle), 0.5, 0.5, 0.5, true)
            end
            if ausPaket then
                GameTooltip:AddLine(L.TT_BP_FROM_PKG, 0.4, 0.8, 1)
            end
...
function F:Refresh()
    ...
    f.spec:SetText(specName or L.NO_SPEC)
    ...
    for _, statKey in ipairs(SK.STAT_KEYS) do
        ...
        if naechster then
            ...
            z.hinweis:SetText((L.HINT_NEXT_DR):format(
                zahl(naechster.fehlt), naechster.faktorDanach * 100))
            z.hinweis:SetTextColor(fr, fg, fb)
        else
            z.balken:SetValue(1)
            z.hinweis:SetText(L.HINT_HARD_CAP)
    ...
    for _, e in ipairs(eintraege) do
        gezeigt = gezeigt + 1
        local z = ListenZeile(f, gezeigt)
        z:ClearAllPoints()
        z:SetPoint("TOPLEFT", RAND, -y)
        z.eintrag = e

        local ziel   = SK.Rechner.ZielRating(e)
        local rating = SK.Spieler.Rating(e.stat)
        local titel  = SK.BPText(e, "titel", e.herkunft == "Paket")

        if not ziel then
            z.text:SetText((L.BP_UNREACHABLE):format(titel, SK.STAT_NAMEN[e.stat]))
        elseif rating >= ziel then
            z.text:SetText((L.BP_REACHED):format(titel, SK.STAT_NAMEN[e.stat], zahl(ziel)))
        else
            z.text:SetText((L.BP_REMAINING):format(
                zahl(ziel - rating), titel, SK.STAT_NAMEN[e.stat], zahl(ziel)))
        end
    ...
    if gezeigt == 0 then
        local z = ListenZeile(f, 1)
        z:ClearAllPoints()
        z:SetPoint("TOPLEFT", RAND, -y)
        z.eintrag = nil
        z.text:SetText(L.BP_NONE)
        z:Show()
        gezeigt, y = 1, y + 16
    end
    ...
    f.fuss:SetText((L.FOOTER_DATA):format(
        meta.patch, meta.stand,
        meta.istPaket and L.FOOTER_PACKAGE or ""))
...
function F:Show()
    ...
    self:Refresh()
    self.frame:Show()
    StatCompassDB.fenster.shown = true
end

function F:Hide()
    if not self.frame then return end
    self.frame:Hide()
    StatCompassDB.fenster.shown = false
end
